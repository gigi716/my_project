...
int get_block_domain(char* block_domain[],int row_cnt);                                                           						//성공하면 체크할 domain 개수 반환
int is_block_domain(const u_char* domain, const int domain_len, const int bolck_domain_num,char* block_domain[]);  							//block해야하면 0 아니면 0아닌 값
void free_get_block_domain(int cnt,char *block_domain[]);
...
char* block_domain[256];
int bolck_domain_num = 0;
...
int main(int argc, char *argv[])
{
	...
	free_get_block_domain(bolck_domain_num, block_domain);
...
void got_packet(u_char *args, const struct pcap_pkthdr *header,
	...
	bolck_domain_num = get_block_domain(block_domain, bolck_domain_num);
	
	result = is_block_domain(domain, domain_len, bolck_domain_num, block_domain);
...
int get_block_domain(char* block_domain[256], int row_cnt){
	
	char* new_select_query = NULL;
	int new_row_cnt;
	...
	if(row_cnt == 0)
    {
		...
		for ( int i = 0; i < row_cnt; i++){
			block_domain[i] = malloc(256);
			if ( block_domain[i] == NULL ) {
		...
		{
			new_select_query = malloc(100);
			sprintf( new_select_query , "SELECT domain FROM blocked_domain_list ORDER BY created_at DESC LIMIT %d", (new_row_cnt - row_cnt) );
			mysql_query( db_connection , new_select_query );
			result = mysql_store_result(db_connection);

			while(row_cnt < new_row_cnt)
			{	
				printf("block_domain : %s\n",block_domain[row_cnt-1]);
				block_domain[row_cnt] = malloc(256);
				if ( block_domain[row_cnt] == NULL ) {
					fprintf(stderr, "ERROR: malloc fail !!\n");	
				}
				row=mysql_fetch_row(result);
				printf("dddd : %s\n",block_domain[row_cnt]);
				strcpy(block_domain[row_cnt] ,row[0]);
				row_cnt++;
			}
		}
		else
...
int is_block_domain(const u_char* domain, const int domain_len, const int bolck_domain_num, char* block_domain[bolck_domain_num]){
	if(domain_len) {
		int cmp_ret = 1;

		for ( int i = 0; i < bolck_domain_num; i++) {
			int str1_len = strlen(block_domain[i]);
			...
			cmp_ret = strcmp(domain, block_domain[i]);

			if(cmp_ret == 0){
				printf("INFO: this domain is blocked \n");
				return cmp_ret; 
			}
		} //end for loop 1.
...
void free_get_block_domain(int bolck_domain_num,char *block_domain[bolck_domain_num]){
	int i;
	for(i=0; i < bolck_domain_num; i++)
    if ( block_domain[i] != NULL )
		{
			free(block_domain[i]);
			block_domain[i] = NULL;
		} else {
			fprintf(stderr, "CRIT : block_domain"
